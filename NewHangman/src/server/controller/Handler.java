		...
		try {
			...
			while ((str = reader.readLine()) != null) {
				...
				if (str.equals("start")) {
					...
					sb.append(ReceiveWordFromServer()).append(',')
							.append("Guess the word!").append(',')
				...
				// When the correct word is found
					...
					sb.append(currWord).append(',')
							.append("Congratulations! You won!").append(',')
							.append(Remaining).append(',').append(score);
					con.passdatatogui(sb);
				}
					...
					if (Remaining == 0) {
						score--;
						StringBuilder sb = new StringBuilder();
						sb.append(currWord).append(',').append("Game Over!")
								.append(',').append(Remaining).append(',')
								.append(score);
						con.passdatatogui(sb);
					} else {
